...
// ─── SAMPLE TRADE DATA ───────────────────────────────────────────────────────
const TRADES = [
  { id:1,  date:"2026-01-05", pair:"EUR/USD", session:"London", setup:"FVG Entry",        outcome:"Win",       pnl:68,   r:2.1,  risk:1, sopOk:true,  exec:9 },
  { id:2,  date:"2026-01-06", pair:"GBP/USD", session:"NY",     setup:"Order Block",      outcome:"Loss",      pnl:-32,  r:-1,   risk:1, sopOk:true,  exec:7 },
  { id:3,  date:"2026-01-07", pair:"XAU/USD", session:"London", setup:"Liquidity Sweep",  outcome:"Win",       pnl:95,   r:3.0,  risk:1, sopOk:true,  exec:8 },
  { id:4,  date:"2026-01-08", pair:"EUR/USD", session:"NY",     setup:"OTE Fib",          outcome:"Breakeven", pnl:0,    r:0,    risk:1, sopOk:true,  exec:6 },
  { id:5,  date:"2026-01-12", pair:"GBP/JPY", session:"London", setup:"FVG Entry",        outcome:"Win",       pnl:112,  r:3.5,  risk:1, sopOk:true,  exec:10 },
  { id:6,  date:"2026-01-13", pair:"EUR/USD", session:"NY",     setup:"Breaker Block",    outcome:"Loss",      pnl:-30,  r:-1,   risk:1, sopOk:false, exec:4 },
  { id:7,  date:"2026-01-14", pair:"USD/JPY", session:"London", setup:"Order Block",      outcome:"Win",       pnl:54,   r:1.7,  risk:1, sopOk:true,  exec:7 },
  { id:8,  date:"2026-01-15", pair:"XAU/USD", session:"London", setup:"FVG Entry",        outcome:"Win",       pnl:88,   r:2.8,  risk:1, sopOk:true,  exec:9 },
  { id:9,  date:"2026-01-19", pair:"GBP/USD", session:"NY",     setup:"Liquidity Sweep",  outcome:"Loss",      pnl:-31,  r:-1,   risk:1, sopOk:true,  exec:6 },
  { id:10, date:"2026-01-20", pair:"EUR/USD", session:"London", setup:"OTE Fib",          outcome:"Win",       pnl:76,   r:2.4,  risk:1, sopOk:true,  exec:8 },
  { id:11, date:"2026-01-21", pair:"GBP/JPY", session:"NY",     setup:"FVG Entry",        outcome:"Win",       pnl:104,  r:3.2,  risk:1, sopOk:true,  exec:9 },
  { id:12, date:"2026-01-22", pair:"XAU/USD", session:"London", setup:"Order Block",      outcome:"Loss",      pnl:-29,  r:-1,   risk:1, sopOk:true,  exec:5 },
  { id:13, date:"2026-02-02", pair:"EUR/USD", session:"London", setup:"FVG Entry",        outcome:"Win",       pnl:81,   r:2.5,  risk:1, sopOk:true,  exec:9 },
  { id:14, date:"2026-02-03", pair:"GBP/USD", session:"NY",     setup:"Breaker Block",    outcome:"Win",       pnl:62,   r:2.0,  risk:1, sopOk:true,  exec:8 },
  { id:15, date:"2026-02-04", pair:"USD/JPY", session:"London", setup:"Liquidity Sweep",  outcome:"Loss",      pnl:-30,  r:-1,   risk:1, sopOk:false, exec:3 },
  { id:16, date:"2026-02-09", pair:"XAU/USD", session:"London", setup:"FVG Entry",        outcome:"Win",       pnl:130,  r:4.1,  risk:1, sopOk:true,  exec:10 },
  { id:17, date:"2026-02-10", pair:"EUR/USD", session:"NY",     setup:"Order Block",      outcome:"Win",       pnl:58,   r:1.8,  risk:1, sopOk:true,  exec:7 },
  { id:18, date:"2026-02-11", pair:"GBP/JPY", session:"London", setup:"OTE Fib",          outcome:"Loss",      pnl:-31,  r:-1,   risk:1, sopOk:true,  exec:6 },
  { id:19, date:"2026-02-16", pair:"GBP/USD", session:"NY",     setup:"FVG Entry",        outcome:"Win",       pnl:73,   r:2.3,  risk:1, sopOk:true,  exec:8 },
  { id:20, date:"2026-02-17", pair:"XAU/USD", session:"London", setup:"Liquidity Sweep",  outcome:"Win",       pnl:118,  r:3.7,  risk:1, sopOk:true,  exec:9 },
  { id:21, date:"2026-02-23", pair:"EUR/USD", session:"London", setup:"Breaker Block",    outcome:"Win",       pnl:66,   r:2.1,  risk:1, sopOk:true,  exec:8 },
  { id:22, date:"2026-02-24", pair:"USD/JPY", session:"NY",     setup:"Order Block",      outcome:"Loss",      pnl:-30,  r:-1,   risk:1, sopOk:true,  exec:5 },
  { id:23, date:"2026-03-02", pair:"GBP/USD", session:"London", setup:"FVG Entry",        outcome:"Win",       pnl:87,   r:2.7,  risk:1, sopOk:true,  exec:9 },
  { id:24, date:"2026-03-03", pair:"XAU/USD", session:"London", setup:"Liquidity Sweep",  outcome:"Win",       pnl:110,  r:3.4,  risk:1, sopOk:true,  exec:10 },
];
...
export default function TradingDashboard() {
  ...
  const avgR      = +(TRADES.reduce((s,t)=>s+t.r,0)/TRADES.length).toFixed(2);
  const avgWinR   = +(wins.reduce((s,t)=>s+t.r,0)/wins.length).toFixed(2);
  const avgLossR  = +(Math.abs(losses.reduce((s,t)=>s+t.r,0)/losses.length)).toFixed(2);
  const expectancy= +((winRate/100*avgWinR)-((100-winRate)/100*avgLossR)).toFixed(3);
  const profFactor= +(wins.reduce((s,t)=>s+t.pnl,0)/Math.abs(losses.reduce((s,t)=>s+t.pnl,0))).toFixed(2);
  ...
  // By Pair
  const byPair  = useMemo(()=>groupBy(TRADES,t=>t.pair),[]);
  const bySetup = useMemo(()=>groupBy(TRADES,t=>t.setup),[]);
  const bySess  = useMemo(()=>groupBy(TRADES,t=>t.session),[]);
    ...
    <div style={{ background: C.bg, minHeight:"100vh", fontFamily:"'DM Sans', sans-serif", color: C.text, padding:"0 0 40px" }}>
      ...
      <div style={{ borderBottom:`1px solid ${C.border}`, padding:"20px 28px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, background:C.bg, zIndex:10 }}>
        ...
        <div style={{ textAlign:"right" }}>
          ...
          <p style={{ color:C.sub, fontSize:11, margin:"2px 0 0" }}>{TRADES.length} trades logged • 2026</p>
        </div>
      </div>
      ...
      <div style={{ padding:"24px 28px" }}>
        ...
        {view==="overview" && <>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))", gap:12, marginBottom:28 }}>
            ...
            <StatCard label="Trades" value={TRADES.length} sub="2026 Demo" />
        ...
        {view==="monthly" && <>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"20px 16px" }}>
            <SectionHeader title="📋 Monthly Scoreboard" sub="Running total toward $50,000 target" />
            <div style={{ marginBottom:16 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                <span style={{ color:C.sub, fontSize:12 }}>Progress to $50,000 target</span>
                <span style={{ color:C.accent, fontSize:12, fontFamily:"'DM Mono',monospace" }}>{fmt$(totalPnL)} / $50,000</span>
              </div>
              <div style={{ background:C.border, borderRadius:999, height:6 }}>
                <div style={{ background:`linear-gradient(90deg,${C.green},${C.accent})`, borderRadius:999, height:"100%", width:`${Math.min(totalPnL/500,100)}%`, transition:"width 1s ease" }}/>
              </div>
              <span style={{ color:C.sub, fontSize:11, marginTop:4, display:"block" }}>{(totalPnL/500).toFixed(1)}% of annual target</span>
            </div>
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
              <thead>
                <tr style={{ borderBottom:`1px solid ${C.border}` }}>
                  {["Month","Trades","Win %","P&L","Running Total","Phase"].map(h=>(
                    <th key={h} style={{ textAlign:"left", color:C.sub, fontWeight:500, padding:"8px 10px", fontSize:11, textTransform:"uppercase", letterSpacing:"0.05em" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {monthlyData.reduce((acc,d)=>{
                  ...
                  <tr key={d.month} style={{ borderBottom:`1px solid ${C.border}` }}>
                    ...
                    <td style={{ padding:"10px" }}>
                      <span style={{ background:`${C.blue}22`, color:C.blue, borderRadius:6, padding:"2px 8px", fontSize:11, fontWeight:600 }}>Demo</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div style={{ padding:"0 28px", borderTop:`1px solid ${C.border}`, paddingTop:16, marginTop:8 }}>
        <p style={{ color:C.muted, fontSize:11, textAlign:"center" }}>
          Replace sample data with your real trades from the Notion Trade Log · Built for Alemseged 2026 · Target: $50,000 USD
        </p>
      </div>
    </div>
  );
